...
### Assumed current draws (per component)

| Component                                | Bare ESP32-WROOM module | DOIT DevKit V1 | Source                                                       |
|------------------------------------------|-------------------------|----------------|--------------------------------------------------------------|
| ESP32 SoC, active, CPU 240 MHz, RF off   | ~80 mA                  | ~100 mA        | [ESP32 datasheet][1] §"Current Consumption Characteristics"  |
| ESP32 SoC, Light Sleep                   | ~0.8 mA                 | (see below)    | [ESP32 datasheet][1] §"RF Power Consumption Specifications"  |
| ESP32 SoC, Deep Sleep (timer wake)       | ~10 µA                  | (see below)    | [ESP32 datasheet][1] §"RF Power Consumption Specifications"  |
| DHT22 / AM2302, measuring                | ~1.5 mA                 | ~1.5 mA        | [Aosong AM2302 datasheet][2] §"Electrical Characteristics"   |
| DHT22 / AM2302, standby                  | ~50 µA                  | ~50 µA         | [Aosong AM2302 datasheet][2] §"Electrical Characteristics"   |
| DOIT DevKit board overhead (CP2102+LDO)  | n/a                     | ~20 mA         | NXP/Silabs CP2102 datasheet + AMS1117 datasheet (typ. quiescent) |
| Battery                                  | 2500 mAh                | 2500 mAh       | Per lab spec                                                 |

[1]: https://www.espressif.com/sites/default/files/documentation/esp32_datasheet_en.pdf
[2]: https://cdn-shop.adafruit.com/datasheets/Digital+humidity+and+temperature+sensor+AM2302.pdf

**DHT22 averaging note.** The sensor draws ~1.5 mA only during the actual single-wire transaction (~5 ms per cycle). Over a 60-second cycle that's `1.5 mA × 5 ms / 60 s ≈ 0.125 µA` — negligible. The dominant DHT22 term is the **continuous ~50 µA standby**, because the sensor's `VCC` is tied to the always-on `3V3` rail and is therefore *not* gated by Deep Sleep. The DHT22 is awake even when the ESP32 isn't.

**LED note.** The two indicator LEDs add some active-phase current (typically ~3 mA each with a ~470 Ω series resistor at 3.3 V). They are deliberately driven OFF during the sleep window, so they contribute zero to `I_sleep`. The exact value depends on the series resistor choice (the `diagram.json` Wokwi part does not model one), so LEDs are left out of the numerical calculation below but would add roughly `(3 mA × t_active_LED) / t_total` to the average — order of magnitude **0.1 mA** on the 60-second Deep Sleep cycle and **0.6 mA** on the 3-second Light Sleep cycle.

The DOIT DevKit `I_sleep` is dominated by the on-board CP2102 USB-UART bridge (~10 mA quiescent) and the AMS1117-3.3 LDO quiescent (~5-10 mA). These parts are why "ESP32 dev boards drain a battery in days" is a common pitfall; on a stripped-down custom PCB the figures drop to the bare-module column.

### Calculation - bare ESP32-WROOM, Light Sleep (`src/main1.cpp`, 3 s cycle)

Per-cycle charge (60 s reformulated as 3 s):

```
Q_cycle = I_ESP_active * t_active
        + I_ESP_lightsleep * t_sleep
        + I_DHT22_standby  * t_total           ; DHT22 is always on

I_avg   = Q_cycle / t_total

I_avg = (80 mA * 0.4 s + 0.8 mA * 2.6 s + 0.050 mA * 3 s) / 3 s
      = (32 + 2.08 + 0.15) / 3
      = 34.23 / 3
      ~= 11.41 mA

Battery life = 2500 mAh / 11.41 mA ~= 219 h ~= 9.1 days
```

The DHT22 contribution is dwarfed by the ESP32's active-phase current at this cadence — the 3-second cycle keeps the chip awake ~13 % of the time, and that dominates everything.

### Calculation - bare ESP32-WROOM, Deep Sleep (`src/archive/main2.cpp`, 60 s cycle)

```
I_avg = (80 mA * 1 s + 0.010 mA * 59 s + 0.050 mA * 60 s) / 60 s
      = (80 + 0.59 + 3.0) / 60
      = 83.59 / 60
      ~= 1.393 mA

Battery life = 2500 mAh / 1.393 mA ~= 1795 h ~= 74.8 days (~2.5 months)
```

This is the case where the DHT22's contribution actually matters. The chip itself averages `(80 + 0.59) / 60 ≈ 1.343 mA`; adding the DHT22's `50 µA` continuous standby bumps that to `~1.393 mA` — about `4 %` shorter battery life than the chip-only figure would suggest. For deployments aiming at sub-100-µA average, the sensor is no longer free.

### Calculation - DOIT DevKit V1, Deep Sleep (no hardware mods)

```
I_avg = (100 mA * 1 s + 20 mA * 59 s + 0.050 mA * 60 s) / 60 s
      = (100 + 1180 + 3.0) / 60
      = 1283 / 60
      ~= 21.38 mA

Battery life = 2500 mAh / 21.38 mA ~= 117 h ~= 4.87 days
```

On the DevKit the DHT22 disappears into the noise floor of the CP2102 + AMS1117 quiescent currents — they swamp it by a factor of ~400.

### What the gaps mean

- **Light vs Deep Sleep on a bare module:** ~8x difference at the chosen
  cadences (219 h vs 1795 h). Light Sleep loses both on its higher sleep
...
- **DHT22's 50 µA standby is the floor:** even an idealised firmware that
  spent zero time awake would still draw ~50 µA continuously just to
  keep the sensor responsive. For battery-critical applications where
  the duty cycle is very low, switching the sensor's VCC through a
  GPIO so it can be powered down between reads is the next optimisation.
